...
export function ReadingChallenge({
  ...
  challenge: { goal: number; current: number; year: number };
}) {
  ...
  const calculatedPercentage =
    challenge.goal > 0 ? (challenge.current / challenge.goal) * 100 : 0;
  const percentage = isNaN(calculatedPercentage)
    ? 0
    : Math.min(100, Math.round(calculatedPercentage));
